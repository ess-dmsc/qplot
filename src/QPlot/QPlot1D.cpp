...
void Multi1D::setMarkers(const QList<Marker1D>& markers)
{
  markers_ = markers;
}
...
{
  highlight_.clear();
  highlight_.push_back(a);
  highlight_.push_back(b);
}

QSet<double> Multi1D::selectedMarkers() const
{
  QSet<double> selection;
...
void Multi1D::plotTitle()
{
  if (showTitle() && !title_text_.isEmpty())
  {
    ...
    floatingText->setPositionAlignment(static_cast<Qt::AlignmentFlag>(Qt::AlignTop) | static_cast<Qt::AlignmentFlag>(Qt::AlignHCenter));
...
QCPItemText* Multi1D::addLabel(QCPItemTracer* crs, const Marker1D& marker)
{
  ...
  markerText->setPositionAlignment(static_cast<Qt::AlignmentFlag>(Qt::AlignHCenter) | static_cast<Qt::AlignmentFlag>(Qt::AlignBottom));
